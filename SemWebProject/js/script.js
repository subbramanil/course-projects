...
var query_surgeryTypes = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> " +
    ...
    "WHERE " +
    "{ " +
    "GRAPH <http://localhost:3030/dataset/vethealth/SurgeryTypes> " +
    "{ "+
    "?sub :state ?state. " +
    "?sub :city ?city. " +
...
var query_emergBeds = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> " +
    ...
    "WHERE { " +
    "GRAPH <http://localhost:3030/dataset/vethealth/Infrastructure> " +
    "{ "+
    "?sub :state ?state." +
    "?sub :city ?city. " +
    "?sub :facility_name ?fac. " +
...
var query_loadInfraData = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> " +
    ...
    "}" +
    "GRAPH <http://localhost:3030/dataset/vethealth/Accreditionstatus> " +
    "{ "+
    "?sub2 :state ?state. " +
    "?sub2 :city ?city. " +
...
var query_loadPatientsByRace = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> " +
    ...
    "WHERE { " +
    "GRAPH <http://localhost:3030/dataset/vethealth/PatientsTypebyRace> " +
    "{ "+
    "?sub1 :state ?state. " +
...
var query_loadPatientsByType = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> " +
    ...
    "PREFIX xsd: <http://www.w3.org/2001/XMLSchema#> " +
    "SELECT ?state " +
    "(sum(xsd:int(?disabled_p1_p4_inpatient)) as ?disabled_p1_p4_inpatient_count) " +
    "(sum(xsd:int(?geriatric_65_yo_inpatient)) as ?geriatric_65_yo_inpatient_count) " +
    "(sum(xsd:int(?disabled_p1_p4_outpatient)) as ?disabled_p1_p4_outpatient_count) " +
    "(sum(xsd:int(?geriatric_65_yo_outpatient)) as ?geriatric_65_yo_outpatient_count) " +
    "(sum(xsd:int(?homeless_outpateint)) as ?homeless_outpateint_count) " +
    "(sum(xsd:int(?mental_health_outpatient)) as ?mental_health_outpatient_count) " +
    "(sum(xsd:int(?non_mental_health_outpatient)) as ?non_mental_health_outpatient_count) " +
    "WHERE { " +
    "GRAPH <http://localhost:3030/dataset/vethealth/PatientsType> " +
    "{ " +
        "?sub :state ?state." +
